...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  // Ramp speed up
	    for (uint16_t speed = 0; speed <= 999; speed += 100) {
	    ...
	    // Ramp speed down
	    for (uint16_t speed = 999; speed >= 100; speed -= 100) {
	      __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, speed);
	      HAL_Delay(500);
	    }
